...
if __name__ == "__main__":
    x = np.load(IRIS_DIR + "/iris_features.npy", allow_pickle=True)
    y = np.load(IRIS_DIR + "/iris_labels.npy", allow_pickle=True)
    N = 120 
    x_train = x[:N]; x_test = x[N:]
    y_train = y[:N]; y_test = y[N:]
    xa_train=np.load(IRIS_DIR + "/iris_train_features_augmented.npy", allow_pickle=True)
    ya_train=np.load(IRIS_DIR + "/iris_train_labels_augmented.npy", allow_pickle=True)
    xa_test =np.load(IRIS_DIR + "/iris_test_features_augmented.npy", allow_pickle=True)
    ya_test =np.load(IRIS_DIR + "/iris_test_labels_augmented.npy", allow_pickle=True)
    ...
    print("SVM (linear, C=1.0, not_aumented):")
